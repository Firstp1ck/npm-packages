...
- `skills/subagent-governance/SKILL.md` — routing, scope boundary, reference router, and the portable workflow with its core admissibility invariants inline.
...
Use when the parent is deciding whether to delegate to one or more children, what outcomes and plan-backed worker contracts to declare, how to sequence or isolate writers, how to react to a failed or partial delegated run, or what to do with reviewer output. Do not route launch syntax and tool arguments, agent or workflow authoring, cost and status inspection without a pending decision, direct implementation work, or feature completion gates.
