...
function parseDirectory(bytes: Uint8Array, limits: DocxLimits): ZipEntryMetadata[] {
  ...
  if (eocd + 22 + commentLength > bytes.length || centralOffset + centralSize > eocd) fail("INVALID_PACKAGE", "ZIP central-directory bounds are invalid.");
...
export class SafeZipArchive {
  ...
  static fromEntries(input: Map<string, Uint8Array>, overrides?: Partial<DocxLimits>): SafeZipArchive { const limits = mergeLimits(overrides), entries = new Map<string, ZipEntry>(); for (const [raw, value] of input) { const p = canonicalPath(raw), data = new Uint8Array(value); entries.set(p, { path: p, data, compressedSize: data.length, uncompressedSize: data.length, crc32: crc32(data), method: 0, flags: 0, localOffset: 0 }); } return new SafeZipArchive(entries, limits); }
