...
export function classifyRelationshipType(type: string): PartClassification | undefined {
  ...
  if (/(?:chart|diagram|customXml|altChunk|glossaryDocument)$/i.test(type)) return "protected";
  return undefined;
}
export function classifyPart(partPath: string, contentType?: string, relationshipTypes: string[] = []): PartClassification {
  ...
  if (/vbaProject|activeX|customUI|oleObject|embeddings\/|attachedTemplate|macrosEnabled/i.test(value)) return "active-content";
