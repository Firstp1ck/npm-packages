...
import { DOCX_ERROR_CODES, DocxError, fail, type DocxErrorCode } from "../errors.ts";
...
export async function resolveSidecar(): Promise<Launch | undefined> {
  ...
  if (configured) { const target = path.resolve(configured); if (!await exists(target)) fail("DEPENDENCY_MISSING", `PI_DOCX_ENGINE_PATH does not exist: ${target}`); if (target.toLowerCase().endsWith(".dll")) { const dotnet = await runCommand("dotnet", ["--info"], { timeoutMs: 5000 }); if (!dotnet.ok) fail("DEPENDENCY_MISSING", "PI_DOCX_ENGINE_PATH points to a DLL but the dotnet runtime is unavailable."); return { command: "dotnet", args: [target], source: "environment-dll" }; } return { command: target, args: [], source: "environment-executable" }; }
...
function parseResponse(stdout: string): EngineResponse { const line = stdout.trim().split(/\r?\n/).filter(Boolean).at(-1); if (!line) fail("PROTOCOL_ERROR", "Open XML sidecar returned no JSON response."); try { const response = JSON.parse(line) as EngineResponse; if (!response || typeof response !== "object" || typeof response.protocolVersion !== "string" || typeof response.ok !== "boolean") fail("PROTOCOL_ERROR", "Open XML sidecar returned an invalid response envelope."); return response; } catch (error) { if (error instanceof DocxError) throw error; fail("PROTOCOL_ERROR", "Open XML sidecar returned invalid JSON."); } }
function protocolErrorCode(value: string | undefined): DocxErrorCode { return value && (DOCX_ERROR_CODES as readonly string[]).includes(value) ? value as DocxErrorCode : "PROTOCOL_ERROR"; }
export class OpenXmlSidecar {
  ...
  private async invoke(launch: Launch, request: EngineRequest, signal?: AbortSignal, timeoutMs = 120_000): Promise<EngineResponse> { const result = await runOwnedProcess(launch.command, launch.args, { stdin: JSON.stringify(request), signal, timeoutMs }); if (result.code !== 0 && !result.stdout.trim()) fail("PROTOCOL_ERROR", `Open XML sidecar exited with code ${result.code}: ${result.stderr || "no stderr"}`); const response = parseResponse(result.stdout); if (response.protocolVersion !== DOCX_CONTRACT_VERSION) fail("PROTOCOL_ERROR", `Unsupported sidecar protocol ${response.protocolVersion}.`); if (!response.ok) { const code = protocolErrorCode(response.error?.code); throw new DocxError(code, response.error?.message ?? "Open XML sidecar failed.", { ...response.error?.details, ...(code === "PROTOCOL_ERROR" && response.error?.code ? { sidecarErrorCode: response.error.code } : {}) }); } if (result.code !== 0) fail("PROTOCOL_ERROR", `Open XML sidecar returned success but exited with code ${result.code}.`); return response; }
