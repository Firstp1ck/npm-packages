...
export const WORKSPACE_FILES_LIVE_WATCH_DEBOUNCE_MS = 250;
export const WORKSPACE_FILES_LIVE_WATCH_MAX_WAIT_MS = 2_000;
...
export function createWorkspaceFilesLiveWatcher({
  watch = watchDirectory,
  debounceMs = WORKSPACE_FILES_LIVE_WATCH_DEBOUNCE_MS,
  maxWaitMs = WORKSPACE_FILES_LIVE_WATCH_MAX_WAIT_MS,
  onChange = () => {},
  onError = () => {},
} = {}) {
  const roots = new Map();
  ...
  const maximumWaitMs = Math.max(0, Number(maxWaitMs) || WORKSPACE_FILES_LIVE_WATCH_MAX_WAIT_MS);
  ...
  function scheduleChange(root) {
    const record = roots.get(root);
    if (!record) return;
    record.tabIds = new Set(record.tabs);
    if (record.timer) clearTimeout(record.timer);
    const now = Date.now();
    ...
    record.timer = setTimeout(() => {
      ...
      current.tabIds.clear();
      try {
        onChange({ root, tabIds, changedAt: new Date().toISOString() });
      } catch (error) {
        reportError(root, error);
      }
  ...
  function ensureRoot(root) {
    ...
    const record = { root, tabs: new Set(), tabIds: new Set(), watcher: null, timer: null, burstStartedAt: 0 };
