...
When agents exchange direct Intercom messages or use native subagent-supervisor coordination, one tag per conversation appears beneath the composer. Select a tag to open a read-only chat view showing the two agent names or IDs and their messages. Attachment contents, tool output, and reasoning are not shown.
