...
When session history is loading, a small status row appears directly above the Git footer. It names the current step, such as **Checking for new messages…**, **Loading conversation history…**, or **Preparing your conversation…**. Existing transcript content and controls remain available; no popup interrupts your work.
