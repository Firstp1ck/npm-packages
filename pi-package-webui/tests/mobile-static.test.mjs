...
assert.match(app, /function triggerGitFooterRefreshFromButton\(\)[\s\S]*requestGitFooterWebuiPayload\(tabContext, \{ force: true, silent: false \}\)/, "git footer refresh button should force a visible /git-footer-refresh request");
...
assert.match(app, /function requestGitFooterWebuiPayload\(tabContext = activeTabContext\(\), \{ force = false, silent = true \} = \{\}\)[\s\S]*?!force && statusEntries\.has\(GIT_FOOTER_WEBUI_STATUS_KEY\)/, "git footer payload refresh should support forced refresh even when a live payload already exists");
