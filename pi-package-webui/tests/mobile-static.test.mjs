...
assert.match(css, /\.widget-area:has\(\.workflow-widget\) \{[\s\S]*?flex:\s*0 1 auto;[\s\S]*?max-height:\s*min\(34rem, 52dvh\);[\s\S]*?overflow:\s*auto/, "workflow widgets should stay within a scrollable output-sized slot so the composer and footer remain visible");
...
assert.match(app, /workflowInspectorMinimizedByTab\.delete\(tabId\)/, "closing a terminal tab should discard its Workflow inspector minimize state");
...
assert.match(css, /\.workflow-inspector-widget\.minimized \{[\s\S]*\.workflow-inspector-minimize-button/, "Workflow inspector should style its minimized state and toggle control");
