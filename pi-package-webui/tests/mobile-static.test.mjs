...
assert.match(app, /function renderTerminalTab\(tab\)[\s\S]*createTerminalTabSessionSummaryButton\(tab\)[\s\S]*function renderTerminalTabGroupItem\(tab, group\)[\s\S]*createTerminalTabSessionSummaryButton\(tab\)/, "regular and grouped Pi terminal tabs should render the shared per-tab summary action");
assert.match(app, /function terminalTabControlKey\(node\)[\s\S]*terminal-tab-summary-button[\s\S]*terminal-tab:\$\{tabId\}:summary/, "per-tab summary actions should retain a stable focus continuity key");
assert.match(app, /\.terminal-tab-close, \.terminal-tab-summary-button, \.terminal-tab-group-add/, "summary actions should not initiate terminal-tab dragging");
...
assert.match(css, /\.terminal-tab-summary-button \{[\s\S]*flex: 0 0 2rem[\s\S]*\.terminal-tab-summary-button:hover:not\(:disabled\)[\s\S]*\.terminal-tab-group-item > \.terminal-tab-summary-button[\s\S]*\.terminal-tab-close \{/, "tab summary actions should stay compact and visually distinct from grouped-tab close actions");
assert.match(css, /@media \(max-width: 720px\)[\s\S]*\.terminal-tab-summary-button,[\s\S]*\.terminal-tab-close \{[\s\S]*min-width: 44px/, "touch layouts should preserve separate 44px summary and close targets");
