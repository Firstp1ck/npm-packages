...
assert.match(app, /function guidedGitPromptCompanionAvailable\(tabId\)[\s\S]*hasLoadedRpcCommand\("git-staged-msg", \{ tabId \}\)/, "guided Git must guard the full browser workflow on the originating tab's prompt companion");
