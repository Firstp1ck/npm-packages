...
assert.match(html, /id="summaryHeaderButton"[^>]*aria-label="Open session summary"[^>]*hidden[\s\S]*id="summaryActionButton"[^>]*data-composer-action-id="summary"[^>]*hidden/, "catalog-gated summary actions should exist in both the terminal header and mobile composer sheet");
...
assert.match(app, /function renderSessionSummaryControls\(\)[\s\S]*hasAvailableCommand\("summary", \{ tabId: activeTabId \}\)[\s\S]*hasAvailableCommand\("summary-setup", \{ tabId: activeTabId \}\)[\s\S]*button\.hidden = !available/, "summary controls should be active-tab command-catalog gated");
