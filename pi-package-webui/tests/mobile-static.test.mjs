...
assert.match(css, /body\.mobile-tabs-expanded \.terminal-tabs-shell \{[\s\S]*?position:\s*fixed;[\s\S]*?left:\s*0\.45rem;[\s\S]*?width:\s*min\(22rem, calc\(100vw - 3rem\)\);[\s\S]*?flex-direction:\s*column/, "expanded mobile terminal navigation should open as a left-side drawer");
...
assert.match(css, /body\.mobile-tabs-expanded \.terminal-tabs \{[\s\S]*?position:\s*static;[\s\S]*?flex-direction:\s*column;[\s\S]*?overflow-y:\s*auto;/, "expanded mobile terminal tabs should use the canonical vertical left-sidebar interaction model");
