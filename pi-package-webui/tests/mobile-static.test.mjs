...
assert.match(app, /async function publishGitWorkflowRepository\(tabId, failure\)[\s\S]*const repoName = cleanGitHubRepoNameInput\(failure\?\.data\?\.repoName \|\| defaultGitInitRepoName\(targetTab\)\);[\s\S]*Publish this repository to GitHub as \$\{repoName\} with the authenticated GitHub CLI \(gh\) account\?[\s\S]*if \(!publishRequested\) return false;[\s\S]*visibility = promptGitPublishVisibility\(repoName\);[\s\S]*if \(!visibility\) return false;/, "guided git publication should derive the repository name and ask only for explicit visibility");
...
assert.match(server, /async function finishGitWorkflowGeneration\(tab, record[\s\S]*restoreGitWorkflowGenerationProfile\(tab, record\)/, "server should restore the active tab model and effort after Guided Git generation finishes");
...
assert.match(app, /function setStreamingThinkingText\(text, \{ complete = false \} = \{\}\)[\s\S]*?const thinking = visibleThinkingText\(text\);[\s\S]*?if \(!thinkingOutputVisible \|\| !thinking\) return false;[\s\S]*?return true;/, "live thinking text setters should ignore empty text instead of clearing or flashing the card");
...
assert.match(app, /function syncStreamingThinkingFromUpdate\(event, update[\s\S]*?update\.type === "thinking_end"\) \{[\s\S]*?const fallback = streamingThinkingTextFallback\(event\);[\s\S]*?setStreamThinkingRawText\(fallback, \{ reconcile: true \}\);[\s\S]*?return render \? setStreamingThinkingText\(streamThinkingRawText \|\| placeholder, \{ complete: true \}\) : true;/, "live thinking end should replace deltas with the final partial-message thinking content");
