...
assert.match(server, /function requireResourceScope\(value\)[\s\S]*scope !== "session" && scope !== "global"/, "resource APIs should accept only session and global scopes");
assert.match(server, /writeWebuiSettings\(\{ resourceDefaults: \{ tools: \{ enabledTools \} \} \}\)/, "global tool choices should persist in Web UI settings");
assert.match(server, /writeWebuiSettings\(\{ resourceDefaults: \{ skills: \{ enabledSkills \} \} \}\)/, "global skill choices should persist in Web UI settings");
assert.match(app, /function nativeResourceScopeControl\(scope,[\s\S]*Session only[\s\S]*Global default/, "Tools and Skills Setup should expose session/global scope choices");
assert.match(app, /body: \{ enabledTools: \[\.\.\.enabledTools\], scope \}/, "Tools Setup should submit its selected persistence scope");
assert.match(app, /body: \{ enabledSkills: \[\.\.\.enabledSkills\], scope \}/, "Skills Setup should submit its selected persistence scope");
assert.match(app, /Future sessions inherit it; this session is unchanged\./, "global selector feedback should explain precedence");
