...
const legacySectionHeights = normalizeUiLayout({
  version: 1,
  sidePanel: { ...valid.sidePanel, sectionHeights: { files: 360 } },
});
assert.equal("sectionHeights" in legacySectionHeights.sidePanel, false, "legacy saved section heights should be ignored during normalization");
...
assert.throws(
  () => validateUiLayoutPatch({ sidePanel: { sectionHeights: { files: 240 } } }),
  /unsupported field: sectionHeights/,
  "removed section-height preferences must no longer be accepted",
);
