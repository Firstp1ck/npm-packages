...
const denseGitFileRowStart = css.indexOf("\n.git-side-panel-file {\n  grid-template-columns", denseGitRowsStart);
assert.notEqual(denseGitFileRowStart, -1, "missing compact Git file-row grid rule");
const denseGitFileRow = ruleBlock(".git-side-panel-file", denseGitFileRowStart);
assert.equal(propertyValue(denseGitFileRow, "grid-template-columns"), "1rem minmax(4rem, 1fr) auto auto", "Git file rows should keep status, filename, stats, and overflow actions on one line");
