...
assert.match(css, /\.composer-intercom-tags \{[\s\S]*display: grid;[\s\S]*grid-auto-columns: minmax\(0, 1fr\);[\s\S]*flex: 0 1 17rem;[\s\S]*max-width: min\(17rem, 100%\);/, "conversation tags should share a compact bounded width without a horizontal scroller");
assert.match(css, /@media \(max-width: 720px\)[\s\S]*\.composer-intercom-tags \{ flex: 1 1 100%; min-width: 0; max-width: 100%; \}/, "conversation tags should regain the available row width on narrow and coarse-pointer layouts");
...
assert.match(html, /\/styles\.css\?v=126/, "the stylesheet URL should advance for the bounded conversation tags");
assert.match(html, /data-app-src="\/app\.js\?v=147"/, "the guarded app URL should advance for incremental viewer wiring");
assert.match(serviceWorker, /const CACHE_NAME = "pi-webui-pwa-v111"/, "the PWA cache identity should advance with browser assets");
