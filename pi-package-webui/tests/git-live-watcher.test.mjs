...
const manager = createGitLiveWatcher({
  watch: fakeWatch,
  debounceMs: 20,
...
const firstRoot = path.resolve("/tmp/pi-webui-live-one");
const secondRoot = path.resolve("/tmp/pi-webui-live-two");

assert.equal(manager.subscribe("tab-a", firstRoot), true);
assert.equal(manager.subscribe("tab-b", firstRoot), true);
assert.equal(created.length, 1, "tabs in one repository should share one recursive watcher");
...
const maxWaitManager = createGitLiveWatcher({
  watch: fakeWatch,
  debounceMs: 80,
  maxWaitMs: 40,
...
assert.equal(manager.subscribe("tab-b", secondRoot), true, "moving a tab should subscribe its new repository");
...
const throwingManager = createGitLiveWatcher({
  watch: () => {
...
console.log("git-live-watcher.test.mjs passed");
