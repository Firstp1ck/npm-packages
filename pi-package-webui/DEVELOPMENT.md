...
- `@firstpick/pi-extension-git-guided-workflow` — preferred cross-surface `/git-guided-workflow` launcher; WebUI consumes its transient start request and keeps the browser renderer/server workflow local.
- `@firstpick/pi-prompts-git-pr` — generated Guided Git commit messages, branch names, and pull-request text.
...
When `/git-guided-workflow` is absent but tab-local `/git-staged-msg` is present, `startLegacyGuidedGitWorkflowFallback` provides the user-approved temporary direct-launch migration path and records an informational event. It is intentionally one named seam for later removal. If the workflow command is present, no direct fallback is allowed. The full existing browser flow requires tab-local `/git-staged-msg`; branch and PR generation additionally recheck `/git-branch-name` and `/pr` at their action boundaries. Missing generation commands are attributed to `@firstpick/pi-prompts-git-pr`, not the workflow extension.
