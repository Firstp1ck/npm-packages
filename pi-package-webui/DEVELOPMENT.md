...
- `tests/control-visibility-static.test.mjs` and `tests/browser/control-visibility.spec.mjs`: stable visibility catalog, Send exclusion, class-based runtime-gating composition, grouped/direct context menus, keyboard behavior, local/durable persistence, tag categories, composer reflow, and recovery actions.
