...
- `tests/browser/control-deck-side-panels.spec.mjs`: placement matrix, right/left/both Control Deck resizing, Sidebar right-rail resizing, cross-side keyboard movement, independent collapse/width persistence, overlay, singleton ARIA, and reload.
...
- Subagent payload v1 normalization preserves only the public recovery contract: `source: "recovered"`, boolean `provisional`/`controllable`, and fleet `version`/`totalActive`/`omitted` aggregates. Raw fleet entries, prompts, and paths are not copied to the HTTP overview. Recovered or otherwise non-controllable provisional rows are status-only at both browser and server boundaries: no output request, cancel, dismiss, clear-finished selection, or retained terminal materialization is permitted. Browser overview refreshes use one shared in-flight promise plus a queued bit; any refresh request received during a request guarantees a trailing fetch before the promise settles, including timer, visibility-resume, cancel, and dismiss callers. `tests/subagent-reliability-integration.test.mjs` covers normalization/privacy, interaction gating, static rendering contracts, and overlapping-refresh state.
