...
The browser-native tool and skill selectors, and the TUI `/tools` and `/skills` commands, offer **Session only**, **Global default**, and **Model default** scopes. In the TUI resource list, type to search, use the arrow keys to move, press `Enter` to toggle, `Ctrl+A` to enable matching entries, `Ctrl+X` to clear matching entries, and `Ctrl+S` to save. `Escape` cancels; `Ctrl+C` clears a non-empty search before cancelling. These shortcuts match `/scoped-models`, including custom Pi bindings for the shared model-selection actions. **Model default** uses a separate searchable provider, ID, and display-name picker. It opens at the active model when available, marks existing profiles, and returns the exact provider/model pair without changing the active model. Tools and skills resolve independently: a session choice wins over an exact case-sensitive provider/model profile, which wins over the global default and then Pi's runtime default.
