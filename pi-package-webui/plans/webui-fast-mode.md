...
The sidebar **Controls → Output processing** group provides the primary server-scoped `Normal`/`Fast` selector with an Apply button and persisted/effective/source/override status. `/settings` also displays the same persisted control in **Browser workflow** with a `server` badge. Both explain that compact controls new/auto-negotiated WebUI connections, CLI/env may temporarily override it, and rich final output is restored. Neither adds the setting to `nativeSettingsPayload()` or `SettingsManager`.
...
| Sidebar Controls selector correction | **Accepted and complete** | User correction added **Controls → Output processing** with `Normal`/`Fast`, Apply, server metadata, retryable failure handling, PWA cache bump, README guidance, and static coverage. Review run `9228d8bb-81ae-4175-b801-693ce08301bc` used runtime `kimi-k3` high and `gemini-3-flash-preview` high; both found no blocker. Full 46-file suite and ratio/hash gate passed. |
...
- **Configuration API and user controls:** authenticated `GET`/`PUT /api/webui-output-mode` returns or writes `{ persistedDefault, effectiveDefault, source, overridden }`. PUT is remote-auth-gated consistently with existing authenticated settings APIs, persists even under CLI/environment override, and neither restarts Pi nor changes a session. The sidebar **Controls → Output processing** group exposes `Normal`/`Fast` with Apply and live persisted/effective/source/override status; transient save failures remain visible and retryable. `/settings` exposes the same persisted value in **Browser workflow**. Both use only `{ outputModeDefault }`, refresh metadata, and keep the server setting out of the Pi `SettingsManager` payload.
...
| Sidebar Controls correction | `public/index.html`, `public/app.js`, `public/styles.css`, `public/service-worker.js`, `README.md`, `tests/fast-mode-client-static.test.mjs`, this plan and report | Primary Controls selector, API wiring, retry UX, cache bump, documentation, review disposition, and validation. |
...
| Sidebar Controls output default | **Controls → Output processing** shows server-scoped `Normal`/`Fast`, Apply, persisted/effective/source/override metadata, unscoped GET, changed-value PUT/refresh, retryable failure status, and no Pi settings payload coupling. | Static regression, independent sidebar review run `9228d8bb-81ae-4175-b801-693ce08301bc`, API/SSE harness, and all-46 suite PASS. |
| Browser workflow output default | `/settings` retains server-badged **Output processing** with `normal`/`compact-v1` and the same persisted API contract. | Completion-fix run `2aff7a57-6650-40ed-ac50-b78fac97c9f5` static UI regression plus API/SSE harness PASS. |
...
| Sidebar correction review, run `9228d8bb-81ae-4175-b801-693ce08301bc`; runtime reviewers `kimi-k3` high and `gemini-3-flash-preview` high | Reviewed Controls placement/accessibility/copy/cache behavior and API/race/settings/test integration. | **PASS with one accepted minor follow-up.** No blockers. Retry-after-failed-PUT and warning-color observations were accepted and fixed; the selection remains enabled for retry, status stays visible, and static/full-suite evidence passes. |

### Rollout, rollback, and remaining risks

- **Rollout:** default remains `normal`. The primary UI path is sidebar **Controls → Output processing → Fast → Apply**. Equivalent paths are `/settings` → **Browser workflow**, `--output-mode compact-v1`, `PI_WEBUI_OUTPUT_MODE=compact-v1`, or the API. The controls display the persisted setting separately from any effective CLI/environment override. Observe the deterministic ledger and ordinary diagnostics, rather than claiming elapsed-time improvement.
- **Rollback:** choose **Normal** under sidebar **Controls → Output processing** and Apply, use `/settings` → **Browser workflow**, send `PUT /api/webui-output-mode` with `{"outputModeDefault":"normal"}`, or remove a compact CLI/environment override. Idle auto clients change immediately; active auto clients change at a semantic barrier. Explicit compact clients remain explicit until reconnect/requesting normal. Code rollback reverts A and B together; no session/transcript/model/tool/provider data migration is required.
