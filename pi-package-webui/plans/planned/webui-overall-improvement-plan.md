...
| UX-2 | Every fresh tab shows two toasts "sessionPath must stay inside the Pi session directory" (403 from `/api/intercom/conversations`) when `~/.pi` is a symlink: `canonicalSessionPath` realpaths the dir but falls back to the unresolved path for the not-yet-created session file (`lib/session-actions.mjs:15-35`) | reliability / first-run | S | yes — **fixed 2026-08-20**: `canonicalSessionPath` now realpaths the nearest existing ancestor for not-yet-created files (regression test in `tests/session-auth-harness.test.mjs`), and `/api/intercom/conversations` returns an empty projection for not-yet-persisted sessions instead of 404 (regression test in `tests/intercom-conversations-http.test.mjs`) |
