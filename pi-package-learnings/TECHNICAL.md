# Technical reference: Troubleshooting learnings for Pi
...
The package code is separate from the archive data. Your actual LEARNINGS notes live in a directory you choose, for example `~/LEARNINGS`.
...
For example, configure a home-directory archive and daily timer with:

```text
/learnings-setup --dir ~/LEARNINGS --timer 20:00
```
...
```text
~/.pi/agent/learnings.env       # contains LEARNINGS_DIR=/home/you/LEARNINGS
~/.pi/agent/LEARNINGS           # symlink to ~/LEARNINGS
~/LEARNINGS                     # actual archive data
```
...
| `archive/` | Generated by the sync process; do not edit it by hand. |
| `manifest.json` | Generated index metadata; do not edit it by hand. |
...
```text
~/LEARNINGS/hyprsunset-warm-mode-keybind-resets-2026-05-19.md
```
...
```bash
LEARNINGS_DIR="$HOME/LEARNINGS" ./scripts/sync-learnings.py
LEARNINGS_DIR="$HOME/LEARNINGS" ./scripts/summarize-learnings.py
```
