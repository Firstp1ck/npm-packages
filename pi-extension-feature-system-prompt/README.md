...
Before the main agent starts, the extension creates an isolated, tool-free Pi session using the active conversation model. The classifier returns one exact request kind. The feature kinds are `feature_lightweight` and `feature_complex`; either appends a short complexity context followed by `APPEND_FEATURE.md`. Other successful classifications leave the system prompt unchanged.

A narrow local continuation check permits bounded previous-request text and the previous effective kind only for short, explicit follow-ups. Continuations inherit the previous effective kind. Session starts and tree navigation reset that in-memory continuation state.
...
- The classifier receives only bounded current request text and, for likely continuations, bounded previous request text plus its effective classification. It does not receive the main system prompt, project files, prompt-file contents, tools, skills, or persistent classifier state.
...
Classification is advisory routing, not an authorization system. It cannot validate the correctness of local prompt policy, prevent a model from misclassifying a request, or enforce actions outside Pi's normal runtime controls.
