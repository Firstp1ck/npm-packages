...
| `settings_get` / `settings_set` | `values` object with schema-checked keys; `modelOrder` is a deduplicated list of at most 256 exact `provider/model-id` strings | current settings and file path |
...
- `qml/shell.qml` — opaque window with a clamped 148–208 px workspace rail, compact workspace header, centered 820 px conversation column, search, transcript follow-output logic, queues, notices, normal-flow composer with a wrapping response/view control strip and anchored more-options/model/thinking drop-ups beneath it, dialogs, and the smoke driver loader. The 560 × 520 minimum remains the responsive boundary.
...
- `qml/Theme.qml` — the only palette and visual-scale owner. It publishes semantic palette, filled-button, interaction, spacing, typography, radius, desktop-geometry, focus, selection, and motion tokens. Automatic mode uses the live portal value before Qt style hints; saved Light/Dark overrides win, and reduced motion makes app-owned transitions immediate.
- `qml/components/` — `AppButton` (compact hover/pressed/active/disabled/focus states), `StatusBadge`, `NoticeBar`, `Composer` (elevated surface, run modes, attachment chips, completion), `CompletionPopup`, `DropUpPicker` (bounded non-modal picker positioned above a control, with optional filtering, explicit keyboard selection, opt-in drag/keyboard reordering, outside/Escape dismissal, and focus return), `SearchBar`, `EmptyState`, `TranscriptRow` (quiet full-width assistant rows and bounded user/thinking surfaces), `MarkdownBlocks`, `ToolCard`, `TabStrip` (one page-tab implementation with horizontal or vertical orientation, status dot, unread and input badges, close, new tab, open folder).
