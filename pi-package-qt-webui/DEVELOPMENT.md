...
- `qml/Theme.qml` — the only palette owner; semantic tokens separate main, sidebar, panel, control-interaction, composer, status, code, quote, table, thinking, dialog, focus, and search roles. Portal-first light/dark selection remains unchanged.
- `qml/components/` — `AppButton` (compact hover/pressed/active/disabled/focus states), `StatusBadge`, `NoticeBar`, `Composer` (elevated surface, run modes, attachment chips, completion), `CompletionPopup`, `DropUpPicker` (bounded non-modal picker positioned above a control, with optional filtering, explicit keyboard selection, outside/Escape dismissal, and focus return), `SearchBar`, `EmptyState`, `TranscriptRow` (quiet full-width assistant rows and bounded user/thinking surfaces), `MarkdownBlocks`, `ToolCard`, `TabStrip` (one page-tab implementation with horizontal or vertical orientation, status dot, unread and input badges, close, new tab, open folder).
...
- `tests/qml-contract.test.mjs` — static contracts for the QML files, including the clamped rail, vertical canonical tabs, centered conversation, minimum size, response controls below the composer, bounded anchored picker geometry and focus return, semantic visual roles, composer elevation, control interaction/focus states, transcript hierarchy, origin-tab callback rejection, resource request fields, durability alerts, null/empty semantics, explicit scopes, effective sources, sampling reasons, disabled/bounded/accessibility state, the limit cross-check, and untrusted-content rules.
- `tests/qml-smoke.test.mjs` — real Quickshell on the current Wayland session, running the `SmokeDriver` scenario twice (default and `QT_SCALE_FACTOR=2`), asserting every marker, model/thinking drop-up bounds and focus return, delayed resource read/mutation responses across tab switches, the captured Pi/helper commands, tool/skill/sampling profile application, unsupported-value preservation, exact dialog answers, and persisted setting. It skips without a Wayland display or Quickshell.
