...
- `tests/qml-contract.test.mjs` — static contracts for the QML files, including the clamped rail, vertical canonical tabs, centered conversation, minimum size, response controls below the composer, semantic visual roles, composer elevation, control interaction/focus states, transcript hierarchy, origin-tab callback rejection, resource request fields, durability alerts, null/empty semantics, explicit scopes, effective sources, sampling reasons, disabled/bounded/accessibility state, the limit cross-check, and untrusted-content rules.
