...
Status: in progress. Phases 0 and 1 are complete. Phase 2 is complete except for Pi- or Quickshell-blocked queue mutation, pasted images, `!` shell, skill tags, and todo state. Phase 3 is complete except for summaries, close-all, and split views. Phase 4 now includes model, thinking, scoped-model ordering, command-palette, tool-profile, skill-profile, and capability-gated sampling controls; subagent model-slot defaults and optional provider controls remain. Phases 5 and 6 have not started beyond the phase 3 worktree service. Phases 7 and 8 include Events, diagnostics, density, usage, live automatic light/dark behavior, and reduced motion. Phase 9 remains open. The current package check passes 137/137 tests, including the default live flow, focused persisted model-order flow, 200% scaling, and packed installation. The resource-profile and scoped-model-ordering tranches each have an explicit, narrowly scoped waiver of an unavailable provider-diverse review quorum. The broader parity plan remains active.
...
- [x] 2026-08-26 scoped model ordering and visual integration: explicit Pi scopes now support drag and `Ctrl+Shift+Up/Down` ordering backed by a bounded, owner-only `modelOrder` preference. Filtering disables moves, stale tab/picker callbacks cannot save, current identities outrank absent saved identities at the 256-entry bound, and Pi still owns model membership. The integration review found and fixed scrolled-list pointer coordinates plus portal-monitor restart and bounded shutdown. Final verification: `npm run check` 137/137, three live Quickshell tests, `qmllint`, 79-file package dry-run, strict HTML report validation, and diff checks pass. The user explicitly waived only the missing second provider review after both configured OpenRouter reviewers hit the monthly key limit.
...
| Models and thinking | Model picker, thinking-effort picker, scoped model ordering, model cycling, model-specific defaults, capability-gated sampling parameters, and optional Codex Fast mode | Searchable model and thinking pickers, cycling, durable explicit-scope ordering, and capability-gated sampling profiles at session/global/exact-model scope done; model-specific picker defaults and Fast mode remain |
...
| Themes and interface | Built-in and custom themes, semantic syntax colors, density, control visibility, panel placement, durable layout, keyboard resizing, and reduced motion | Portal-first automatic light/dark palette, live theme changes, semantic interaction tokens, compact/comfortable density, persisted display settings, and reduced motion done; custom themes, panel layout, and high contrast remain |
...
- [x] Add durable ordering for Pi's explicit scoped-model list. — drag handles and `Ctrl+Shift+Up/Down` share one bounded move path; filtering disables reordering; exact identities persist without changing Pi scope membership
...
- [x] Respect reduced motion and Qt palette changes. — automatic mode prefers the live XDG portal value with Qt style hints as fallback; reduced motion removes transition and working-indicator animation durations
- [ ] Add a high-contrast mode with tested semantic token overrides.
...
- [ ] Light, dark, high-contrast, and reduced-motion smoke checks pass. — light, dark, automatic restoration, and reduced motion pass; high contrast remains
