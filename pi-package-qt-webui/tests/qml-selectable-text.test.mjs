...
const componentFiles = [
  ...
  "components/SessionList.qml",
  "components/AppButton.qml",
  "Theme.qml",
];

for (const scale of [1, 2]) test(`real Qt selectable text keeps source, bounds, and row actions at ${scale * 100}% scaling`, { timeout: 30_000 }, async (t) => {
  ...
  const directory = await mkdtemp(path.join(os.tmpdir(), "qt-webui-selectable-text-"));
  ...
  await writeFile(path.join(directory, "tst_selectable-text.qml"), await readFile(new URL("fixtures/selectable-text-checks.qml", import.meta.url), "utf8"));
