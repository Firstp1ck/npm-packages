...
# shellcheck source=package-publishable-changes.sh
source "$SCRIPT_DIR/package-publishable-changes.sh"
...
Examples:
  ./dev/scripts/check-publish-readiness.sh
  ./dev/scripts/check-publish-readiness.sh pi-extension-notes
  ./dev/scripts/check-publish-readiness.sh --publisher bun --check-alt-client
  ./dev/scripts/check-publish-readiness.sh --target pi-extension-brave-search --strict-auth
  ./dev/scripts/check-publish-readiness.sh --all
EOF
}
...
if [[ $has_fail -eq 1 ]]; then
  ...
  echo "  ./dev/scripts/bump-package-versions.sh --all"
  echo "  ./dev/scripts/bump-package-versions.sh --all --apply"
  exit 1
fi
